...
class lime_test
{
    ...
    private function equals($exp1, $exp2)
    {
        ...
        } elseif (is_string($exp1) && is_numeric($exp1) || is_string($exp2) && is_numeric($exp2)) {
            return $exp1 == $exp2;
        } elseif (is_string($exp1) || is_string($exp2)) {
    ...
    public function isnt($exp1, $exp2, $message = '')
    {
        ...
        if (!$result = $this->ok(!$value, $message)) {
            $this->set_last_test_errors([
                sprintf("      %s", var_export($exp1, true)), '          ne',
    ...
    public function cmp_ok($exp1, $op, $exp2, $message = '')
    {
        ...
        if (!$this->ok($result, $message)) {
            $this->set_last_test_errors([
                ...
                sprintf("          %s", $op), sprintf("      %s", str_replace("\n", '', var_export($exp2, true)))
            ]);
        }
    ...
    public function isa_ok($var, $class, $message = '')
    {
        ...
        if (!$result = $this->ok($type == $class, $message)) {
            $this->set_last_test_errors([
                sprintf("      variable isn't a '%s' it's a '%s'", $class, $type)
            ]);
        }
    ...
    public function include_ok($file, $message = '')
    {
        if (!$result = $this->ok((@include($file)) == 1, $message)) {
            $this->set_last_test_errors([
                sprintf("      Tried to include '%s'", $file)
            ]);
        }
    ...
    protected function update_stats()
    {
        ...
        list($this->results['tests'][$this->test_nb]['file'], $this->results['tests'][$this->test_nb]['line']) =
            $this->find_caller(debug_backtrace());
    }
...
class lime_harness extends lime_registration
{
    ...
    public function run()
    {
        ...
        $out_dir = $this->options['test_path'].DIRECTORY_SEPARATOR.'lime_test';
        if (!is_dir($out_dir)) {
            mkdir($out_dir, 0777, true);
        }
        $logfile = $out_dir.DIRECTORY_SEPARATOR.'test-'.date('YmdHis').'-'.mt_rand(0, 99999).'.txt';
        ...
        foreach ($this->files as $file) {
            ...
            $filename = 'test-'.mt_rand(0, 99999);
            $test_file = $out_dir.DIRECTORY_SEPARATOR.$filename.'.php';
            $result_file = $out_dir.DIRECTORY_SEPARATOR.$filename.'.out';;
            file_put_contents(
                $test_file,
                <<<EOF
<?php
