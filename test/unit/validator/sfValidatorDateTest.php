...
try {
    ...
    $t->pass('->clean() throws an exception if the date is not within the range provided by the min/max options');
    $t->is($e->getMessage(), 'The date must be after 31/12/1805.', '->clean() check exception message');
}
...
try {
    ...
    $t->pass('->clean() throws an exception if the date is not within the range provided by the min/max options');
    $t->is($e->getMessage(), 'The date must be before 31/12/2107.', '->clean() check exception message');
}
